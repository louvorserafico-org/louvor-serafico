...
Preparar o app para abrir materiais armazenados no Supabase Storage usando links temporarios gerados por Edge Function.
...
Conteudo premium nao deve depender apenas de estado local ou regra no cliente. O fluxo seguro adotado e:

1. usuario autentica;
2. app consulta assinatura real;
3. app chama a Edge Function `create-asset-signed-url`;
4. backend valida assinatura no servidor;
5. backend gera signed URL curta;
6. app abre o arquivo.

Enquanto a assinatura real ainda nao existe, esta etapa deve ser vista como preparacao tecnica e UX de acesso. A protecao final de receita depende da tabela `subscriptions` refletir a assinatura real.
...
- ao tocar, o app chama a Edge Function;
- se Supabase, sessao real ou Edge Function nao estiverem prontos, exibe mensagem clara.
...
- Removemos o helper antigo de chamada direta ao Storage no app para evitar caminho inseguro ou confuso.

## DoD

- [x] Resolver chamada da Edge Function com TDD.
